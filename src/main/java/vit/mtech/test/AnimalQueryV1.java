...
class QueryV1 {
    ...
    /*String queryString="select * where {\n" +
     "?s ?p ?o .\n" +
     "}";*/
    String queryString =  "select ?product ?label ?value1 ?value2 where {\n"
...
//     "}";
    ...
    public void map() {
        ...
        for (int i = 0; i < paramv1.size(); i++) {
            if (paramv1.get(i).equalsIgnoreCase(".\nFILTER")) {
                filter = true;
                fil.add(i);
            ...
            while (j < SubQueries.size()) {
                ...
                if (filter == true) {
                    ...
                    if (filterops == true) {
                        ...
                        so.filterOut(lastfound, qstring[1], qstrings.get(2), qstrings.get(0));
    ...
    private int findTheLastOccurence(String sub, String obj) {
        int lastfound=0;
         for (int xy = 0; xy < SubQueries.size(); xy++) {
                        ...
                        if (sub.equals(SubQueries.get(xy).get(0)) && !obj.equals(SubQueries.get(xy).get(2))) {
                            lastfound = xy;
...
public class AnimalQueryV1 extends Query {

    public static void main(String args[]) {
        QueryV1 m = new QueryV1();
